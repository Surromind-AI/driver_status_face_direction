...
def predict_loader(path, predict_batch_size, num_workers=24, pin_memory=False):
    images_output_dir = mtcnn(path)
    normalize = transforms.Normalize(
        mean=IMAGENET_MEAN, std=IMAGENET_STD)

    predict_dataset = \
        datasets.ImageFolder(images_output_dir,
                             transforms.Compose([
    ...
    return torch.utils.data.DataLoader(predict_dataset,
        batch_size=predict_batch_size, shuffle=False, sampler=None,
        num_workers=num_workers, pin_memory=pin_memory)

def mtcnn(path):
    images_output_dir = "./predict_processing/test/"
    detector = MTCNN()
    files = sorted(os.listdir(path+"/test/"))
    for f in files:
        img = cv2.imread(path+"/test/"+f)
        box = detector.detect_faces(img)
        box = box[0]["box"]
        img = img[box[1]-100:int(box[1])+int(box[3])+100, box[0]-100:int(box[0])+int(box[2])+100]
        cv2.imwrite(images_output_dir + f, cv2.resize(img, (112, 112)))
